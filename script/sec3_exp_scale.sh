...
#SBATCH -o logs/planning_scale/instruct/slurm_scale_test.out
#SBATCH -e logs/planning_scale/instruct/slurm_scale_test.err
#SBATCH -A pi_jensen_umass_edu

# Create logs directory if it doesn't exist
mkdir -p logs/planning_scale/instruct
...
for PROMPT in "${PROMPTS[@]}"; do
  ...
  python -m plan_trace.pipeline ${PROMPT} \
    --max-tokens 50 \
    --save \
    --output-dir outputs/planning_scale/instruct \
